...
Bimo Robotics Kit example API usage with CPG and keyboard control

USAGE: press Q or E key while the robot is walking to steer it
"""
...
PRESSED_KEY = None  # Logs current pressed key
...
def main():
    ...
    while True:
        ...
        # Overwrite orientation Z component (deviation) based on key pressed
        if PRESSED_KEY == 'q':
            bimo.unlock_heading()
            z_dev = -1.0
        ...
        elif bimo.base_heading is None:
            # Sets current heading as target once, after key release
            bimo.lock_heading()
